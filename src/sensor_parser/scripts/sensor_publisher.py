...
def parse_GPS(data_string):
    ...
    # <update_flag: int>\t
    GPS_start = 4
    data = data_string.split("\t")[GPS_start:]
    GPS_msg = NavSatFix()
...
def parse_channel(data_string, channel_number):
    ...
    channels_start = 7
    # channels_end = 8
    data = data_string.split("\t")[channels_start:]
    # print(data)
    channel_msg = UInt32()
...
def parse_IMU(data_string):
    ...
    IMU_start = 0   
    data = data_string.split("\t")[IMU_start:]
    # print(data)
    IMU_msg = Imu()
...
def sensor_publisher(ser):
    ...
    while not rospy.is_shutdown():
        ...
        if len(line.split('\t')) != param_number:
            continue
...
if __name__ == '__main__':
    ...
    ser = serial.Serial()
    ser.baudrate = 115200
    ser.port = "/dev/serial/by-id/usb-Arduino__www.arduino.cc__0042_55834323633351F030F1-if00" # something like this
    ser.timeout = 1
    ser.open()
    print(ser.name)
